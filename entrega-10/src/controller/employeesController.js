...
const getByDepartment = (req, res) => {
  ...
  const employeesByDepartment = employees.filter(
...
const getByMaxWage = (req, res) => {
  ...
  const employeesDataByWages = employees.filter(
    (employee) => employee.wages <= maxWages
  );
  const employeesNameByWages = employeesDataByWages.map(
...
const putEmployee = (req, res) => {
  ...
  employees.splice(index, 1, employeeUpdated);

  fs.writeFile(
    "./src/models/employees.json",
    JSON.stringify(employees),
    "utf8",
    function (err) {
      if (err) {
...
const patchEmployee = (req, res) => {
  ...
  try {
    ...
    Object.keys(update).forEach((key) => {
      employeeToBeUpdated[key] = update[key];
    });
